...
const nama_instansi = 'RS Bunda Medika Jakabaring'; // Hospital Name
const apiUrl = 'http://localhost/login/api/'; // API Server URL
...
var app  = new Framework7({
  ...
  name: 'APAM BUNDA JAKABARING', // App name
  dialog: {
    title: 'Peringatan',
    buttonOk: 'Ya',
    buttonCancel: 'Tidak'
    ...
    $$('.page[data-name="postregister"] .saveregister-btn').on('click', function () {
      ...
      if(kode_validasi == localStorage.getItem("kode_validasi")) {
        ...
        app.request.post(apiUrl + 'apam/', {
          ...
          data = JSON.parse(data);
          if(data.state == "invalid") {
            app.dialog.alert('Gagal menyimpan data pendaftaran. Silahkan ulangi lagi.');
          }
          else if(data.state == "valid") {
...
$$(document).on('page:init', '.page[data-name="home"]', function(e) {
  ...
  if (no_rkm_medis) {
    ...
    var hitungNotif = setInterval(function () {
      app.request.post(apiUrl + 'apam/', {
        action: 'notifikasi',
        no_rkm_medis: no_rkm_medis,
        token: token
        ...
        data = JSON.parse(data);
        $$('.hitung-notifikasi').text(data.length);
      });
      // clearInterval(notifikasi);
    }, 3000);
...
$$(document).on('page:init', '.page[data-name="riwayatlist"]', function(e) {
  ...
  app.request.post(apiUrl + 'apam/', {
    ...
    var html = '';
    for(i=0; i<data.length; i++) {
      html += '<li style="margin: 5px 10px; background-color: white; border-radius: 15px;">';
...
$$(document).on('page:init', '.page[data-name="riwayatdetail"]', function(e) {
  ...
  app.request.post(apiUrl + 'apam/', {
    ...
    for(i=0; i<data.length; i++) {
      ...
      if (data[i]['keluhan']=='' || data[i]['keluhan']==null) {
        html += '  <div class="card-content">Keluhan: Belum Ada </div>';
      } else {
        html += '  <div class="card-content">Keluhan: ' + data[i]['keluhan'] + '</div>';
      }

      if (data[i]['pemeriksaan']=='' || data[i]['nm_penyakit']==null) {
        html += '  <div class="card-content">Pemeriksaan: Belum Ada</div>';
      } else {
      ...
      html += '<div class="card padding">';
      if (data[i]['nm_penyakit']=='' || data[i]['nm_penyakit']==null) {
        html += '  <div class="card-content">Belum Ada</div>';
      } else {
      ...
      html += '<div class="card padding">';
      if (data[i]['nama_brng']=='' || data[i]['nama_brng']==null) {
        html += '  <div class="card-content"> Belum ada </div>';
      } else {
      ...
      html += '<div class="card padding">';
      if(data[i]['pemeriksaan_lab'] == '' || data[i]['pemeriksaan_lab'] == null) {
        html += '  <div class="card-content">Belum Ada</div>';
      } else {
      ...
      html += '<div class="card padding">';
      if(data[i]['hasil_radiologi'] == '' || data[i]['hasil_radiologi'] == null) {
        html += '  <div class="card-content">Belum Ada</div>';
      } else {
      ...
      html += '<div class="card padding">';
      if(data[i]['gambar_radiologi'] == null || data[i]['gambar_radiologi']=='') {
        html += '  <div class="card-content"> Belum Ada</div>';
      } else {
...
$$(document).on('page:init', '.page[data-name="riwayatranap-detail"]', function(e) {
  ...
  app.request.post(apiUrl + 'apam/', {
    ...
    for(i=0; i<data.length; i++) {
      ...
      html += '<div class="block-title">Resep Obat</div>';
      html += '<div class="card padding">';
      if (data[i]['nama_barang']=='' || data[i]['nama_barang']==null) {
        html += '  <div class="card-content">Belum Ada</div>';
      } else {
        html += '  <div class="card-content">' + data[i]['nama_barang'] + '</div>';
      }
      html += '</div>';
...
$$(document).on('page:init', '.page[data-name="kontak-rs"]', function(e) {
  
  $$('.page[data-name="kontak-rs"] .whatsapp').on('click', function () {
    window.open(`https://api.whatsapp.com/send?phone=${whatsapp}`, '_blank')
  })

  $$('.page[data-name="kontak-rs"] .telepon').on('click', function () {
    window.open(`tel:+${telepon}`, '_blank')
  })
